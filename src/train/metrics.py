...
from utils.bioes_decode import bioes_decode
...
class QuerySpanF1(nn.Module):
    ...
    def forward(self, start_preds, end_preds, match_logits, start_label_mask, end_label_mask, match_labels):
        return query_span_f1(
            start_preds, end_preds, match_logits, start_label_mask, end_label_mask, match_labels, flat=self.flat
        )
...
def query_span_f1(start_preds, end_preds, match_logits, start_label_mask, end_label_mask, match_labels, flat=False):
    ...
    match_preds = (
        match_preds & start_preds.unsqueeze(-1).expand(-1, -1, seq_len) & end_preds.unsqueeze(1).expand(-1, seq_len, -1)
    )
    match_label_mask = start_label_mask.unsqueeze(-1).expand(-1, -1, seq_len) & end_label_mask.unsqueeze(1).expand(
        -1, seq_len, -1
    )
...
def extract_flat_spans(start_pred, end_pred, match_pred, label_mask):
    ...
    for start_item in start_positions:
        bioes_labels[start_item] = f"B-{pseudo_tag}"
    for end_item in end_positions:
        bioes_labels[end_item] = f"E-{pseudo_tag}"
        ...
        if match_pred[tmp_start][tmp_end]:
            if tmp_start != tmp_end:
                for i in range(tmp_start + 1, tmp_end):
                    bioes_labels[i] = f"M-{pseudo_tag}"
            else:
                bioes_labels[tmp_end] = f"S-{pseudo_tag}"

    tags = bioes_decode([(pseudo_input, label) for label in bioes_labels])
